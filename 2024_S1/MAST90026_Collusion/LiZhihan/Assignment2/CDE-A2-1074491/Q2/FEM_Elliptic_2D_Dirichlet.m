% solve the model elliptic equation with Dirichlet boundary conditions.
function u = FEM_Elliptic_2D_Dirichlet(node, elem, Dfunc, qFunc, fFunc, gFunc, quad_order)
N = size(node,1); NT=size(elem,1); %Initialization
...
ME = elem_massnew(P1, P2, P3, qFunc, quad_order); % compute local mass matrix
...
b = b-A*v;
v(freeNode) = A(freeNode, freeNode)\b(freeNode); %Solve the Linear System
u=v; % Combine Results
