...
Dfunc = @(x, y) 1;
qFunc = @(x, y) 0;
fFunc = @(x, y) 17 * sin(x) .* cos(4 * y);
gFunc = @(x, y) sin(x) .* cos(4 * y);

appro = FEM_Elliptic_2D_Dirichlet(node, elem, Dfunc, qFunc, fFunc, gFunc, quad_order);
