function chebRunge(arg)
%RUNGEINTERP  Runge's polynomial interpolation example.
%   F(x) = 1/(1+25*x^2)
%   Polynomial interpolation at Chebyshev points, -1 <= x <= 1.
...
if nargin == 0
   ...
   set(gcf,'doublebuffer','on','numbertitle','off', ...
       'name','Runge''s interpolation example using Chebyshev points')
   n = 1;
   u = -1.1:.01:1.1;
   z = F(u);
   ...
   h.minus = uicontrol('units','norm','pos',[.38 .01 .06 .05], ...
          'fontsize',12,'string','<','callback','chebRunge(''n--'')');
   h.n = uicontrol('units','norm','pos',[.46 .01 .12 .05], ...
          'fontsize',12,'userdata',n,'callback','chebRunge(''n=1'')');
